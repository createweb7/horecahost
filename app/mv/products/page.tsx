...
export const metadata: Metadata = {
  ...
  openGraph: {
    ...
    url: `${SITE_ORIGIN}/mv/products`,
    siteName: "HorecaHost",
    type: "website",
    images: [{ url: `/api/og?title=Equipment+for+Maldives&description=500%2B+Products+from+60%2B+Global+Brands`, width: 1200, height: 630 }],
  },
};

export default async function MaldivesProductsPage() {
  ...
  const breadcrumbSchema = {
    ...
    itemListElement: [
      ...
      { "@type": "ListItem", position: 3, name: "Products", item: `${SITE_ORIGIN}/mv/products` },
    ],
  };
      ...
      <main className="min-h-screen bg-white">
        ...
        <section className="relative bg-gray-900 overflow-hidden">
          ...
          <div className="relative mx-auto max-w-6xl xl:max-w-7xl px-6 sm:px-8 pt-6 pb-16">
            ...
            <h1 className="text-4xl sm:text-5xl font-extrabold text-white mb-4 leading-tight">
              Commercial Equipment Available in Maldives
            </h1>
            <p className="text-gray-400 text-lg max-w-2xl">
              Browse our full range of hospitality equipment — all available for delivery to luxury resorts, hotels, and restaurants across the Maldives.
            </p>
          </div>
        </section>
        ...
        <div className="mx-auto max-w-6xl xl:max-w-7xl px-6 sm:px-8 py-12">
          <p className="text-sm text-gray-500 mb-8">
            Showing {products.length} products — <Link href="/contact" className="text-red-600 hover:underline">contact us</Link> for full availability in the Maldives
          </p>

          {products.length > 0 ? (
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
              {products.map((product) => (
                <ProductCard key={product.id} product={product} locale="en" href={`/mv/${product.slug}`} />
              ))}
            </div>
          ) : (
