...
export async function generateMetadata({
  ...
  if (product) {
    const title = `${product.name_en} Supplier in Maldives | HorecaHost`;
    const description = `Buy ${product.name_en} in the Maldives. HorecaHost supplies premium hospitality equipment to resorts, hotels, and restaurants across the Maldives. Enquire today.`;
  ...
  if (category) {
    ...
    const title = override?.metaTitle ?? `${category.name_en} Supplier in Maldives | HorecaHost`;
    const description = override?.metaDescription ?? `HorecaHost supplies premium ${category.name_en.toLowerCase()} to resorts, hotels, and restaurants across the Maldives. 60+ global brands. Enquire today.`;
    const ogTitle = override?.h1 ?? `${category.name_en} Maldives`;
    const ogDesc = override?.heroSubtitle ?? `Premium ${category.name_en} for Maldives Resorts & Hotels`;
    return {
      title,
      description,
      alternates: {
  ...
  if (subcategory) {
    const override = country.seoOverrides?.[slug];
    const title = override?.metaTitle ?? `${subcategory.name_en} Supplier in Maldives | HorecaHost`;
    const description = override?.metaDescription ?? `HorecaHost supplies premium ${subcategory.name_en.toLowerCase()} to resorts, hotels, and restaurants across the Maldives. 60+ global brands. Enquire today.`;
    const ogTitle = override?.h1 ?? `${subcategory.name_en} Maldives`;
    const ogDesc = override?.heroSubtitle ?? `Premium ${subcategory.name_en} for Maldives Resorts & Hotels`;
    return {
      title,
      description,
      alternates: {
  ...
  if (brand) {
    const title = `${brand.name_en} Equipment Supplier in Maldives | HorecaHost`;
    const description = `HorecaHost supplies ${brand.name_en} equipment to resorts, hotels, and restaurants across the Maldives. Enquire today for pricing and availability.`;
