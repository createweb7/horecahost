...
export async function generateMetadata({
  ...
  try {
    ...
    const { data: productData } = await supabase
      .from("products")
      .select(
        "id, name_en, name_ar, slug, images, brand:brands(id, name_en, country_en), category:categories(name_en), subcategory:subcategories(name_en)",
      )
      .eq("slug", slugPath)
      .maybeSingle();
    ...
    if (contentType === "product") {
      ...
      try {
        ...
        if (metadata) {
          ...
          if (anyMetadata) {
            if (anyMetadata.meta_title)
              metaTitle = sanitize(anyMetadata.meta_title);
            if (anyMetadata.meta_description)
              metaDescription = sanitize(anyMetadata.meta_description);
            if (anyMetadata.meta_keywords)
              metaKeywords = sanitize(anyMetadata.meta_keywords)
                .split(",")
                .map((k: string) => k.trim());
          }
        }
      ...
      keywords =
        metaKeywords.length > 0
          ? metaKeywords
          : [brand.name_en, brand.country_en, "horeca equipment"].filter(
              Boolean,
            );
      canonical = `https://www.horecahost.com/${slug.join("/")}`;
    }
        ...
        images: [
          {
            url: ogImage.startsWith("http")
              ? ogImage
              : `https://www.horecahost.com${ogImage}`,
            width: 1200,
            height: 630,
            alt: title,
          },
                  ...
                  ? [
                      {
                        "@type": "ListItem",
                        position: 4,
                        name: product.subcategory.name_en,
                        item: `${SITE_ORIGIN}/${product.subcategory.slug}`,
                      },
                    ]
                  : []),
                {
